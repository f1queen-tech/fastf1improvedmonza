...
delta_time, ref_tel, compare_tel = utils.delta_time(ant_lap, lec_lap)
fig, (ax1, ax2) = plt.subplots(
    2, 1, figsize=(12, 8), sharex=True,
    gridspec_kw={'height_ratios': [3, 1]}
)
ax1.plot(ant_tel['Distance'], ant_tel['Speed'], color="#00FFE1", label='ANT')
ax1.plot(lec_tel['Distance'], lec_tel['Speed'], color='#E8002D', label='LEC')
ax1.set_ylabel('Speed (km/h)')
ax1.set_title('ANT vs LEC — Fastest Lap Speed Trace + Delta — 2025 Italian GP')
ax1.legend()
ax2.plot(ref_tel['Distance'], delta_time, color="#FF6600")
ax2.axhline(0, color='grey', linewidth=0.8, linestyle='--')
ax2.set_xlabel('Distance (m)')
ax2.set_ylabel('Gap (s)\nLEC ahead ^ / ANT ahead v')
